        ...
        <div class="d-flex">
          ...
          <div class="form">
            <h1 class="form__title text-center">
              {{ $t('BOOKING.ADVISOR_BOOKING.MODAL_BOOKING.TITLE') }}
            </h1>
            ...
            <v-form ref="form" v-model="form.valid">
              <div class="form__textField">
                <label class="label"
                  >{{
                    $t('BOOKING.ADVISOR_BOOKING.MODAL_BOOKING.NAME')
                  }}
                  :</label
                >
                <v-text-field
                  :value="$lang(advisor.name_th, advisor.name_en)"
                  type="text"
                  single-line
                  outlined
                  disabled
              ...
              <div class="form__textField">
                ...
                >
                <v-textarea
                  :value="$lang(advisor.detail_th, advisor.detail_en)"
                  type="text"
                  single-line
                  outlined
                  readonly
              ...
              <div class="form__textField">
                <label class="label"
                  >{{
                    $t('BOOKING.ADVISOR_BOOKING.MODAL_BOOKING.BOOKING_DATE')
                  }}
                  :</label
                >
              ...
              <div class="form__textField">
                <label class="label"
                  >{{
                    $t('BOOKING.ADVISOR_BOOKING.MODAL_BOOKING.START_TIME')
                  }}
                  :</label
                >
                ...
                <div class="d-flex">
                  ...
                  <span class="time-label">
                    {{ $t('BOOKING.ADVISOR_BOOKING.MODAL_BOOKING.END_TIME') }}
                  </span>
              ...
              <div class="form__textField">
                <label class="label"
                  >{{
                    $t('BOOKING.ADVISOR_BOOKING.MODAL_BOOKING.PURPOSE')
                  }}
                  :</label
                >
                <v-textarea
              ...
              <div class="d-flex flex-column align-center mt-3">
                <v-btn
                  ...
                  @click="submit()"
                >
                  {{ $t('BOOKING.ADVISOR_BOOKING.MODAL_BOOKING.SUBMIT') }}
                </v-btn>
                ...
                >
                  {{ $t('BOOKING.ADVISOR_BOOKING.MODAL_BOOKING.CANCEL') }}
                </v-btn>
              </div>
            </v-form>
          </div>
...
export default {
  ...
  methods: {
    closeDialog(value) {
      ...
      this.$emit('advisor_booking_dialog', value)
    },
    async submit() {
    ...
    async getData() {
      let res = await this.$store.dispatch('advisor/find', this.id)
      this.advisor = res.data
      this.advisor.detail_th = this.$stripHtml(res.data.detail_th)
      this.advisor.detail_en = this.$stripHtml(res.data.detail_en)
      this.form.value.advisor_id = res.data.id
