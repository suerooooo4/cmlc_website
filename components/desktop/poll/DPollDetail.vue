      ...
      <section class="content-container">
        <v-row>
          <v-col cols="12">
            <h2 class="section-title">
              <span>
                <i class="fa fa-chevron-left" @click="openPage('/poll')"></i>
                {{ $t('POLL.TITLE_DETAIL') }}
              </span>
            </h2>
          </v-col>
        ...
        <v-row v-if="user_poll">
          <v-col>
            ...
            <div v-else>
              <DPollResult v-if="render_result" :id="id" />
            </div>
          </v-col>
        </v-row>
      </section>
...
<script>
import DPollForm from '~/components/desktop/poll/DPollForm.vue'
import DPollResult from '~/components/desktop/poll/DPollResult.vue'
