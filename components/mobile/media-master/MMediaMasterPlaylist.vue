        ...
        <v-slide-group class="a" show-arrows>
          <v-slide-item v-for="(item, index) in category_list" :key="index">
            <v-btn
              ...
              rounded
              @click="
                openCategory('/media-master',{ media_category_id: item.id },index),getMediaSearch(item.id)
              "
            >
      ...
      <v-container>
        <v-row class="pb-3">
          <v-col cols="12">
            ...
            <div v-if="form.is_search" class="media-container">
              <v-row class="pt-3">
                <v-col v-for="media of mediaSearch" :key="media.id" cols="6">
                  <v-card class="media hover-card">
                    <div
                      class="totle_text"
                      @click="openMediaMasterDialog(media.id)"
                    >
                      <img
                      ...
                      <div class="media-infos">
                        ...
                        <p class="media-creator">
                          ...
                          <span class="media-value">
                            {{
                              $lang(
                                media.creator_name_th,
                                media.creator_name_en
                              )
                            }}
                          </span>
                        </p>
            ...
            <div v-else class="">
              <v-row>
                ...
                >
                  <div
                    @click="
                      openPage('/media-master/playlist', {
  ...
  export default {
    ...
    methods: {
      ...
      async getMediaSearch(id) {
        ...
        this.mediaSearch = res.data.data
        this.$refs.infiniteloading.stateChanger.reset()
        this.last_page = res.data.lastPage
        this.total = res.data.total
        this.page++
